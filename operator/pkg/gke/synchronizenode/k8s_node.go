...
func startSynchronizingNodes(config Config, clientset k8sClient.Clientset, lc cell.Lifecycle) {
	...
	for _, nodeLabel := range nodeLabels {
		...
		_, nodeController := informer.NewInformer(
			utils.ListerWatcherWithModifier(
				utils.ListerWatcherFromTyped[*slim_corev1.NodeList](clientset.Slim().CoreV1().Nodes()), nodeOptsModifier,
			),
			&slim_corev1.Node{},
			0,
