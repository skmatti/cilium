...
func TestIlbConvertService(t *testing.T) {
				...
				endpoints: map[string]*Endpoints{
					"cluster2": {
						Backends: map[cmtypes.AddrCluster]*Backend{
							cmtypes.MustParseAddrCluster("192.168.1.1"): {
				...
				endpoints: map[string]*Endpoints{
					"cluster3": {
						Backends: map[cmtypes.AddrCluster]*Backend{
							...
							cmtypes.MustParseAddrCluster("192.168.1.2"): {
								Ports: store.PortConfiguration{
									"": &loadbalancer.L4Addr{
										Protocol: loadbalancer.TCP, Port: 9376,
									}}},
							cmtypes.MustParseAddrCluster("192.168.1.3"): {
								Ports: store.PortConfiguration{
									"": &loadbalancer.L4Addr{
										Protocol: loadbalancer.TCP, Port: 9376,
									}}},
		...
		},
		{
			...
			cacheEndpoints: &externalEndpoints{
				endpoints: map[string]*Endpoints{
					...
					"cluster4": {
						Backends: map[cmtypes.AddrCluster]*Backend{
							cmtypes.MustParseAddrCluster("192.169.1.1"): {
								Ports: store.PortConfiguration{
									"": &loadbalancer.L4Addr{
										Protocol: loadbalancer.TCP, Port: 9376,
									}}},
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			// Seed cache
			id := ServiceID{
				Name:      tc.clusterService.Name,
				Namespace: tc.clusterService.Namespace,
			}
			...
			for ip, port := range tc.clusterService.Backends {
				wantEndpoints.Backends[cmtypes.MustParseAddrCluster(ip)] = &Backend{
					Ports: port,
				}
			}
	...
	}{
		{
			...
			want: ServiceEvent{
				...
				Service: NewService(
					...
					loadbalancer.SVCTrafficPolicyCluster, loadbalancer.SVCTrafficPolicyCluster, 0,
					map[string]string{
						serviceTypeKey:       string(slimv1.ServiceTypeLoadBalancer),
						serviceAnnotationKey: serviceAnnotationValue,
...
func TestMergeServiceUpdateAndDeleteForILB(t *testing.T) {
	...
	clusterService := store.ClusterService{
		...
		Frontends: map[string]store.PortConfiguration{"1.1.1.1": {}},
	...
	backendUpdates := []map[cmtypes.AddrCluster]*Backend{
		{
			cmtypes.MustParseAddrCluster("1.1.1.1"): {Ports: store.PortConfiguration{}},
		},
		{
			cmtypes.MustParseAddrCluster("1.1.1.1"): {Ports: store.PortConfiguration{}},
			cmtypes.MustParseAddrCluster("2.2.2.2"): {Ports: store.PortConfiguration{}},
		},
		{
			cmtypes.MustParseAddrCluster("2.2.2.2"): {Ports: store.PortConfiguration{}},
		},
	...
	for i, backendUpdate := range backendUpdates {
		...
		for ip, be := range backendUpdate {
			backends[ip.Addr().String()] = be.Ports
		}
