...
func (t *testHandler) ProcessFlow(ctx context.Context, p *pb.Flow) {
	labels := t.ContextOptions.GetLabelValues(p)
...
func TestRegister(t *testing.T) {
	...
	t.Run("Should remove metrics series with ContextPod", func(t *testing.T) {
		...
		assert.EqualValues(t, handlers[0].(*testHandler).ProcessCalled, 2)
		...
		assert.EqualValues(t, handlers[0].(*testHandler).ListMetricCalled, 1)
		...
		assert.EqualValues(t, handlers[0].(*testHandler).ListMetricCalled, 2)
