package sfc

import (
	"fmt"
	"net"

	"github.com/cilium/cilium/pkg/bpf"
	"github.com/cilium/cilium/pkg/byteorder"
	"github.com/cilium/cilium/pkg/ebpf"
	"github.com/cilium/cilium/pkg/types"
)

const (
	PathMapName      = "google_sfc_path"
	PathMaxEntries   = 16384
	MaxServicePathId = (1 << 24) - 1
)

// PathKey must match 'struct sfc_path_key' in "bpf/lib/google_sfc.h".
type PathKey struct {
	Path uint32 `align:"path"` // network order
}

// NewPathKey returns a new PathKey instance from SPI, SI values.
// Returns an error if SPI exceeds the maximum value.
func NewPathKey(servicePathId uint32, serviceIndex uint8) (*PathKey, error) {
	if servicePathId > MaxServicePathId {
		return nil, fmt.Errorf("SPI %d exceeds max value %d", servicePathId, MaxServicePathId)
	}
	path := (servicePathId << 8) | uint32(serviceIndex)
	key := &PathKey{Path: byteorder.HostToNetwork32(path)}
	return key, nil
}

func extractSPISI(path uint32) (uint32, uint8) {
	p := byteorder.NetworkToHost32(path)
	return p >> 8, uint8(p)
}

func (key *PathKey) ServicePathId() uint32 {
	spi, _ := extractSPISI(key.Path)
	return spi
}

func (key *PathKey) ServiceIndex() uint8 {
	_, si := extractSPISI(key.Path)
	return si
}

func (key *PathKey) New() bpf.MapKey { return &PathKey{} }
func (key *PathKey) String() string {
	return fmt.Sprintf("(%d, %d)", key.ServicePathId(), key.ServiceIndex())
}

// PathEntry must match 'struct sfc_path_entry' in "bpf/lib/google_sfc.h".
type PathEntry struct {
	Address types.IPv4 `align:"address"`
}

func NewPathEntry(ip net.IP) *PathEntry {
	entry := PathEntry{}
	copy(entry.Address[:], ip.To4())
	return &entry
}

func (entry *PathEntry) New() bpf.MapValue { return &PathEntry{} }
func (entry *PathEntry) String() string    { return entry.Address.String() }

var PathMap = bpf.NewMap(
	PathMapName,
	ebpf.Hash,
	&PathKey{},
	&PathEntry{},
	PathMaxEntries,
	bpf.BPF_F_NO_PREALLOC,
).WithCache()
