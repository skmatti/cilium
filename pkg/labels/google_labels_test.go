...
func TestMergeMultiNICLabels(t *testing.T) {
	...
	// Define a custom comparer that ignores the cidr field
	labelComparer := cmp.Comparer(func(l1, l2 Label) bool {
		return l1.Key == l2.Key && l1.Value == l2.Value && l1.Source == l2.Source
	})
	for _, tc := range testcases {
		t.Run(tc.desc, func(t *testing.T) {
			tc.to.MergeMultiNICLabels(tc.from)
			if diff := cmp.Diff(tc.to, tc.want, labelComparer); diff != "" {
