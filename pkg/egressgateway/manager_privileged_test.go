...
func TestMultigatewayPolicy(t *testing.T) {
	...
	slices.SortStableFunc(nodes, func(a, b testNodes) int {
	...
	for i, ep := range eps {
		newEP, newID := newEndpointAndIdentity(ep.name, ep.ipv4, ep1Labels)
	...
	assignEndpoints := func(endpoints []testEndpoints, gateways []testNodes, ipv4 bool) []egressRule {
		...
		for _, endpoint := range endpoints {
			gEpID := &googleEndpointID{
				NamespacedName: types.NamespacedName{
					Name:      endpoint.ep.GetName(),
					Namespace: endpoint.ep.GetNamespace(),
				},
				clusterID: identity.NumericIdentity(uint32(endpoint.ep.Identity.ID)).ClusterID(),
				ip:        netip.MustParseAddr(endpoint.ipv4),
			}
			h := getEndpointHash(gEpID)
			index := consistentHash32(h, int32(len(gateways)))
			gw := gateways[index]
	...
	// gateways using the hash.
	// Note that this is evaluated from the node1 perspective, so the entries for other nodes will
	// have a zeroIP as EgressIP.
	expectedpolicyMap := assignEndpoints(eps, nodes, true)
	assertEgressRules(t, policyMap, expectedpolicyMap)
	...
	reconciliationEventsCount = waitForReconciliationRun(t, egressGatewayManager, reconciliationEventsCount)
	assertEgressRules(t, policyMap, expectedpolicyMap[1:])
	...
	reconciliationEventsCount = waitForReconciliationRun(t, egressGatewayManager, reconciliationEventsCount)
	expectedpolicyMap = assignEndpoints(eps, nodes, true)
	assertEgressRules(t, policyMap, assignEndpoints(eps, nodes, true))
	...
	addPolicy(t, k.policies, &policy1)
	waitForReconciliationRun(t, egressGatewayManager, reconciliationEventsCount)
	for i := range expectedpolicyMap {
		expectedpolicyMap[i].egressIP = egressIP1
		expectedpolicyMap[i].gatewayIP = nodes[0].ip
	}
	assertEgressRules(t, policyMap, assignEndpoints(eps, nodes[:1], true))
}
...
func tryAssertEgressRules(policyMap egressmap.PolicyMap, rules []egressRule) error {
	...
	for _, r := range parsedRules {
		...
		if policyVal.GetEgressAddr() != r.egressIP {
			return fmt.Errorf("mismatched egress IP. Expected: %s, Got: %s", r.egressIP.String(), policyVal.GetEgressAddr().String())
		}

		if policyVal.GetGatewayAddr() != r.gatewayIP {
			return fmt.Errorf("mismatched gateway IP. Expected: %s, Got: %s", r.gatewayIP.String(), policyVal.GetGatewayAddr().String())
		}
	}
