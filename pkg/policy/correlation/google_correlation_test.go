...
func TestCorrelateMultiplePolicies(t *testing.T) {
	localIP := "1.2.3.4"
	...
	allowAllLabel := utils.GetPolicyLabels("foo-namespace", "allow-all", "9999-9999", utils.ResourceTypeCiliumNetworkPolicy)
	...
	expected := []*flowpb.Policy{
		...
		},
		{
			...
			Labels: []string{
				...
				"k8s:io.cilium.k8s.policy.namespace=foo-namespace",
				"k8s:io.cilium.k8s.policy.uid=9999-9999",
			},
			Revision: 1,
		},
	}
