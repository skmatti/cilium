...
// AnnotatedObject extends runtime.Objects to additionally require GetAnnotations method.
...
func (n *networkPolicyLogger) allowedPoliciesForDelegate(policies []*flow.Policy) []*flow.Policy {
	...
	for _, p := range policies {
		key := resource.Key{Name: p.Name, Namespace: p.Namespace}
		var annotated bool
		switch p.Kind {
		case "networkpolicy", "NetworkPolicy":
			annotated = objAnnotationTrue(n.stores.NetworkPolicyStore, key, AnnotationEnableAllowLogging)
		...
		default:
			log.
				...
				WithField("kind", p.Kind).
				Debug("Policy kind not supported")
...
// objAnnotationTrue fetches the object from the provided store and checks if the annotation value is true.
func objAnnotationTrue[T AnnotatedObject](store resource.Store[T], resourceKey resource.Key, annotationKey string) bool {
	item, exists, err := store.GetByKey(resourceKey)
	if err != nil {
		log.
			WithField("kind", fmt.Sprintf("%T", new(T))).
			WithField("key", resourceKey).
			Errorf("Failed to fetch object: %v", err)
		policyLoggingErrorCount.WithLabelValues(errorReasonGetPolicy).Inc()
		return false
	}
	if !exists {
	...
	return item.GetAnnotations()[annotationKey] == "true"
}

func (n *networkPolicyLogger) shouldLogNamespace(name string) bool {
	return objAnnotationTrue(n.stores.NamespaceStore, resource.Key{Name: name}, AnnotationEnableDenyLogging)
}
