...
func NewNodeFirewallAgent(kubeClient kubernetes.Interface, nodeFWClient nnpclient.Interface, ciliumPolicyManager types.PolicyManager) *NodeFirewallAgent {
	...
	nc := &NodeFirewallAgent{
		eventRecorder:       broadcaster.NewRecorder(nnpscheme.Scheme, apiv1.EventSource{Component: "anet-node-firewall-agent", Host: nodeTypes.GetName()}),
...
func (nc *NodeFirewallAgent) sync(key string) error {
	...
	if !exists || policy.DeletionTimestamp != nil {
		...
		opts := ciliumpolicy.DeleteOptions{Source: metrics.LabelEventSourceK8s}
		if _, err := nc.ciliumPolicyManager.PolicyDelete(policyLabels, &opts); err != nil {
