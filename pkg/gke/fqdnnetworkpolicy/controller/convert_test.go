...
func testNewPolicyRepository() *policy.Repository {
	...
	repo := policy.NewPolicyRepository(identityCache, nil, nil, nil, api.NewPolicyMetricsNoop())
...
func TestDNSProxyRedirectKubeDNS(t *testing.T) {
	...
	repo.AddListLocked(api.Rules{rule})
...
func TestPolicyLabels(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if diff := cmp.Diff(want, got, cmpopts.SortSlices(labelLess), cmpopts.IgnoreUnexported(ciliumLbls.Label{})); diff != "" {
...
func TestParseFQDNNetworkPolicy(t *testing.T) {
	...
	defaultDenyConfig := api.DefaultDenyConfig{
		Ingress: ptr.To(false),
		Egress:  ptr.To(true),
	}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			opts := cmp.Options{
				...
				cmpopts.IgnoreUnexported(api.EndpointSelector{}, api.EgressCommonRule{}, ciliumLbls.Label{}),
			}
