...
func NewController(kubeClient kubernetes.Interface, slimClient slimclientset.Interface, redirectServiceClient versioned.Interface, redirectPolicyManager RedirectPolicyManager, iptablesManager *iptables.Manager) (*Controller, error) {
	...
	_, c.podController = informer.NewInformer(
		...
		cache.ResourceEventHandlerFuncs{
			...
			UpdateFunc: func(old, curr interface{}) { c.updateNoTrackHandler(old, curr) },
...
// addNoTrackHandler handles inserting NOTRACK rules when the pod is added.
func (c *Controller) addNoTrackHandler(curr interface{}) {
	c.mutex.Lock()
	...
	// Only handle NodeLocalDNS pods
	labels := newPod.ObjectMeta.Labels
	if val := labels[redirectpolicy.KeyNodeLocalDNS]; val != redirectpolicy.LabelNodeLocalDNS && val != redirectpolicy.LabelNodeLocalDNSDPv2 {
		return
	}
...
func (c *Controller) updateNoTrackHandler(old, curr interface{}) {
	c.mutex.Lock()
	defer c.mutex.Unlock()
...
// DelNoTrackForQueuedEndpoint handles removal of NO TRACK rules for queued endpoint deletes.
func (c *Controller) DelNoTrackForQueuedEndpoint(ep redirectpolicy.DeletedEndpointMetadata) {
	c.mutex.Lock()
	defer c.mutex.Unlock()
	...
	val, found := ep.Labels[redirectpolicy.KeyNodeLocalDNS]
	if found && (val == redirectpolicy.LabelNodeLocalDNS || val == redirectpolicy.LabelNodeLocalDNSDPv2) {
		c.removeNoTrackRules(ep.IP)
	}
}
...
func (c *Controller) addNoTrackRules(ip string) {
	scopedLog := log.WithFields(logrus.Fields{
		logfields.IPAddr: ip,
		logfields.Port:   redirectpolicy.PortNodeLocalDNS,
	})
	addr, err := netip.ParseAddr(ip)
	if err != nil {
		scopedLog.Warnf("Cannot install NOTRACK rules for %v:%v", ip, redirectpolicy.PortNodeLocalDNS)
		return
	}
	scopedLog.Debug("Upserting NOTRACK rule")
	c.iptablesManager.InstallNoTrackRules(addr, redirectpolicy.PortNodeLocalDNS)
}
...
func (c *Controller) removeNoTrackRules(ip string) {
	...
	addr, err := netip.ParseAddr(ip)
	if err != nil {
		scopedLog.Warnf("Cannot remove NOTRACK rules for %v:%v", ip, redirectpolicy.PortNodeLocalDNS)
		return
	}
	scopedLog.Debug("Removing NOTRACK rules")
	c.iptablesManager.RemoveNoTrackRules(addr, redirectpolicy.PortNodeLocalDNS)
}
...
func (c *Controller) clearNodeLocalDNSRedirect(o *v1alpha1.RedirectService) {
	...
	localPodIPs, err := c.redirectPolicyManager.GetLocalPodsForPolicy(lrpConfig)
	if err != nil {
		log.Errorf("%v", err)
		return
	}

	for _, ip := range localPodIPs {
