...
func (m fakeMap) Update(sourceIP netip.Addr, destCIDR netip.Prefix, egressIP, gatewayIP netip.Addr) error {
	k := key{sourceIP.String(), destCIDR.String()}
	if _, ok := m[k]; ok {
		return fmt.Errorf("Update() called with duplicated key: %#v", k)
	}
	m[k] = gatewayIP.String()
	return nil
}

func (m fakeMap) Delete(sourceIP netip.Addr, destCIDR netip.Prefix) error {
	k := key{sourceIP.String(), destCIDR.String()}
	if _, ok := m[k]; !ok {
...
func TestAddDelTSConfig(t *testing.T) {
	...
	m.podIPs = map[netip.Addr]struct{}{
...
func TestAddDelPodIP(t *testing.T) {
	...
	if err := m.addPodIP(netip.MustParseAddr("192.168.1.1")); err != nil {
	...
	if err := m.addPodIP(netip.MustParseAddr("192.168.1.2")); err != nil {
		t.Fatalf("m.addPodIP(192.168.1.1) returns error: %v", err)
	}
	want[key{"192.168.1.2", "10.1.1.0/24"}] = "10.1.1.1"
	want[key{"192.168.1.2", "10.1.2.0/24"}] = "10.1.1.1"
	...
	m.delPodIP(netip.MustParseAddr("192.168.1.1"))
...
func TestParse(t *testing.T) {
	...
	ipNet1 := netip.MustParsePrefix("10.1.1.0/24")
	ipNet2 := netip.MustParsePrefix("10.1.2.0/24")
	tests := []struct {
		name        string
		dstCIDRs    []string
		nextHop     string
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if diff := cmp.Diff(got, tc.want, cmp.AllowUnexported(tsConfig{}), cmpopts.EquateComparable(netip.Addr{})); diff != "" {
