...
func ipOf(pod *slim_corev1.Pod) (netip.Addr, error) {
	ip, err := netip.ParseAddr(pod.Status.PodIP)
	if err != nil {
		return netip.IPv4Unspecified(), fmt.Errorf("invalid pod IP %q: %v", pod.Status.PodIP, err)
	}
	if !ip.Is4() {
		return netip.IPv4Unspecified(), fmt.Errorf("doesn't support non-ipv4 pod %q: %s", pod.GetName(), pod.Status.PodIP)
	}
	return ip, nil
}
...
func (c *Controller) handlePod(curr interface{}) {
	...
	pod := informer.CastInformerEvent[slim_corev1.Pod](curr)
	if pod == nil {
		return
	}
...
func (c *Controller) delPod(obj interface{}) {
	...
	pod := informer.CastInformerEvent[slim_corev1.Pod](obj)
	if pod == nil {
		return
	}
	scopedLog := log.WithField("name", pod.Name).WithField("namespace", pod.Namespace)
...
func (c *Controller) handleNode(curr interface{}) {
	...
	currNode := informer.CastInformerEvent[corev1.Node](curr)
	if currNode == nil {
		return
	}
