...
func NewController(clientset *versioned.Clientset, wgAgent types.WireguardAgent, ipcache IPCache) *Controller {
...
func (c *Controller) Start(ctx context.Context) error {
	...
	if !cache.WaitForNamedCacheSync("remoteNodeController", ctx.Done(), c.informer.HasSynced) {
		return fmt.Errorf("RemoteNode informer failed to sync")
	}
	return nil
}

func (c *Controller) Stop() {
	log.Info("Shutting down remote node controller")
	close(c.stopCh)
}
