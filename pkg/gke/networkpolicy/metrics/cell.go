...
func registerNetpolMetrics(lc cell.Lifecycle, flowPlugin gkeflow.FlowPlugin, dc *option.DaemonConfig, config Config, registry *metrics.Registry) {
	if !dc.EnableHubble {
		return
	}
	...
	exporter := newExporter(flowPlugin.Dispatcher, registry)
	lc.Append(cell.Hook{
