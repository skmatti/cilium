...
var _ = Describe("Sample MultiCluster", Label("sample-multicluster"), Ordered, func() {
	...
	BeforeAll(func() {
		...
		artifacts, ok := os.LookupEnv("ARTIFACTS_BASE")
		Expect(ok).To(BeTrue())
		Expect(artifacts).NotTo(BeEmpty())

		clusterInfoList, err := artifact.NewClusterInfoList(artifacts)
		Expect(err).NotTo(HaveOccurred())

		clientsets, err = clusterInfoList.NewClientsets()
		Expect(err).NotTo(HaveOccurred())
	})
	...
	It("can list nodes on all clusters", func() {
		for idx, clientset := range clientsets {
			...
			Expect(err).NotTo(HaveOccurred())
			klog.Infof("Found %d nodes on (%s).", len(nodes.Items), clusters[idx].Name)
