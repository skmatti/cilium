...
PATCH_CONTENT_DIR=${ROOT}/${PATCH_CONTENT_DIR:-addon/patch_content/abm-1.30.0-gke.1829/overlays/image-only}
  ...
  gcp-gke)
    ...
    if [[ -n "${CILIUM_GITREF:-}" ]]; then
      make -C "${ROOT}" \
        ...
        ADVANCEDDATAPATH_BASE_IMAGE_TAG="${ADVANCEDDATAPATH_BASE_IMAGE_TAG:-}" \
        IMAGE_REGISTRY="${IMAGE_REGISTRY}" \
        CILIUM_DOCKER_IMAGE_TAG="${CILIUM_DOCKER_IMAGE_TAG}" \
        TBCONFIG="$(realpath "${TBCONFIG}" || true)" \
        CILIUM_GITREF="${CILIUM_GITREF}" \
