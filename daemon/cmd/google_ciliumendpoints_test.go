...
func Test_cleanStaleCEPinCESWhenMultiNIC(t *testing.T) {
	...
	tc := map[string]struct {
		...
		"Multi-NIC CEPs with local pods with endpoints should not be GCd": {
			...
			managedEndpoints:   map[string][]*endpoint.Endpoint{"y/bar": {&endpoint.Endpoint{IPv4: netip.MustParseAddr("1.2.3.4"), ID: 1}}},
			expectedDeletedSet: []string{},
		},
		"Multi-NIC CEPs with local pods with IPv6 endpoints should not be GCd": {
			ciliumEndpointSlice: &v2alpha1.CiliumEndpointSlice{
				Endpoints: []v2alpha1.CoreCiliumEndpoint{multinicCoreCEP("foo-net1-1234", node.GetCiliumEndpointNodeIP(), "", "2002::1234:abcd:ffff:c0a8:101")},
				Namespace: "x",
			},
			managedEndpoints:   map[string][]*endpoint.Endpoint{"y/bar": {&endpoint.Endpoint{IPv6: netip.MustParseAddr("2002::1234:abcd:ffff:c0a8:101"), ID: 1}}},
			expectedDeletedSet: []string{},
		},
		"Multi-NIC CEPs with local pods without endpoints should be GCd": {
			ciliumEndpointSlice: &v2alpha1.CiliumEndpointSlice{
			...
			managedEndpoints:   map[string][]*endpoint.Endpoint{"y/bar": {&endpoint.Endpoint{IPv4: netip.MustParseAddr("5.6.7.8"), ID: 1}}},
			expectedDeletedSet: []string{"x/foo-net1-1234"},
		},
		"Non local Multi-NIC CEPs should not be GCd": {
			ciliumEndpointSlice: &v2alpha1.CiliumEndpointSlice{
