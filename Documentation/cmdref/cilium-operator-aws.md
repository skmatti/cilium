...
```
  ...
  -h, --help                                                  help for cilium-operator-aws
      ...
      --operator-enable-metrics-server-tls                    Enable mTLS for operator metrics server
      --operator-metrics-server-tls-cert-file string          Path to the public key file for the operator metrics server. The file must contain PEM encoded data.
      --operator-metrics-server-tls-client-ca-files strings   Paths to one or more public key files of client CA certificates to use for TLS with mutual authentication (mTLS). The files must contain PEM encoded data. When provided, this option effectively enables mTLS.
      --operator-metrics-server-tls-key-file string           Path to the private key file for the operator metrics server. The file must contain PEM encoded data.
