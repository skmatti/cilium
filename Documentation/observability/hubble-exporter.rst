...
You can edit configmap to change flow log settings without a need for pod to be
restarted (changes should be reflected within 60s because of configmap
propagation delay):

.. code-block:: shell-session

    kubectl -n kube-system edit cm cilium-flowlog-config
...
settings as static one: ``hubble-export-file-max-size-mb`` & ``hubble-export-file-max-backups``

Sample flow logs configs:
