...
func NewEndpoint(pod *slim_corev1.Pod, allocator IdentityAllocator) (*Endpoint, error) {
	...
	for _, ip := range pod.Status.PodIPs {
		if net.ParseIP(ip.IP).To4() != nil {
			ipv4, err := netip.ParseAddr(ip.IP)
			if err != nil {
				return ep, err
			}
			ep.IPv4 = ipv4
		} else {
			ipv6, err := netip.ParseAddr(ip.IP)
			if err != nil {
				return ep, err
			}
			ep.IPv6 = ipv6
...
func (e *Endpoint) UpdateLabels(ctx context.Context, identityLabels, infoLabels labels.Labels, blocking bool) (regenTriggered bool) {
	...
	rev := e.replaceIdentityLabels(labels.LabelSourceAny, identityLabels)
	e.mutex.Unlock()
	if rev != 0 {
...
func (e *Endpoint) replaceIdentityLabels(source string, l labels.Labels) int {
	if l == nil {
		return e.identityRevision
	}

	changed := e.OpLabels.ReplaceIdentityLabels(source, l, log)
	rev := 0
	if changed {
		e.identityRevision++
		rev = e.identityRevision
...
func getEndpointNetworking(mdlNetworking *models.EndpointNetworking) (networking *cilium_v2.EndpointNetworking) {
	...
	log.WithField("ip", IPv4Address.String()).Info("Set NodeIP")
	networking.NodeIP = IPv4Address.String()
...
var _ cache.IdentityAllocatorOwner = &dummyIdentityAllocatorOwner{}

func (dummyIdentityAllocatorOwner) UpdateIdentities(_, _ identity.IdentityMap) {}
