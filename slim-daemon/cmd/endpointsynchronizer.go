...
type EndpointResourceSynchronizer struct{}
...
func (epSync *EndpointResourceSynchronizer) RunK8sCiliumEndpointSync(e *Endpoint) {
	var (
		endpointID     = e.ID
		controllerName = EndpointSyncControllerName(endpointID)
...
// Cilium agent.
func (epSync *EndpointResourceSynchronizer) DeleteK8sCiliumEndpointSync(e *Endpoint) {
	controllerName := EndpointSyncControllerName(e.ID)
...
func deleteCEP(ctx context.Context, scopedLog *logrus.Entry, ciliumClient v2.CiliumV2Interface, e *Endpoint) error {
	podName := e.GetK8sPodName()
	if podName == "" {
