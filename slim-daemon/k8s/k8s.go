...
func Init() error {
	...
	controller.NewManager().UpdateController("k8s-heartbeat",
		controller.ControllerParams{
			...
			RunInterval: K8sHeartbeatTimeout,
		},
	)

	if err := k8sversion.Update(Client(), true); err != nil {
		return err
	}
...
func setDialer(config *rest.Config) func() {
	if K8sHeartbeatTimeout == 0 {
		return func() {}
	}
	ctx := (&net.Dialer{
		Timeout:   K8sHeartbeatTimeout,
		KeepAlive: K8sHeartbeatTimeout,
	}).DialContext
