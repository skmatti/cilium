...
func runApiserver() error {
	...
	option.BindEnv(vp, option.DisableCiliumNetworkPolicyCRDName)
	...
	option.BindEnv(vp, option.ClustermeshNamespaceLabels)
...
func startServer(startCtx hive.HookContext, clientset k8sClient.Clientset, services resource.Resource[*slim_corev1.Service]) {
	...
	if mockFile != "" {
		...
		if len(cfg.namespaceLabels) > 0 {
			if err := watchNamespaces(clientset, cfg.namespaceLabels); err != nil {
