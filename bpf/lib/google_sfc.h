...
/* SFC_SELECT_STATIC_PREFIX represents the size in bits of the static prefix part of an SFC cidr key. */
#define SFC_SELECT_STATIC_PREFIX (64)
#define SFC_SELECT_PREFIX_LEN(PREFIX) (SFC_SELECT_STATIC_PREFIX + (PREFIX))
#define SFC_SELECT_IPV4_PREFIX SFC_SELECT_PREFIX_LEN(32)
...
// Reserved in go/geneve-registry
#define SFC_GENEVE_OPT_CLASS	0x0132
#define SFC_GENEVE_OPT_TYPE	(GENEVE_OPT_TYPE_CRIT | 0x0d)
...
sfc_select4(struct __ctx_buff *ctx, struct iphdr *ip4, bool is_egress, struct redirect_info *redir)
{
	int ret;
	struct sfc_select_key select_key = {
		.lpm_key = { SFC_SELECT_IPV4_PREFIX, {} },
		.ep_id = LXC_ID,
		.family = ENDPOINT_KEY_IPV4,
		.is_egress = is_egress,
		.protocol = ip4->protocol,
	};
	struct sfc_select_entry *select_entry, *select_entry_all_ports;
	...
	if (is_egress)
		select_key.ip4 = ip4->daddr;
	else
		select_key.ip4 = ip4->saddr;

	select_entry = map_lookup_elem(&SFC_SELECT_MAP, &select_key);
	if (select_entry != NULL)
		redir->path = select_entry->path;


	// Check if "all ports" lookup results in a more specific match
	select_key.port = 0;
	select_entry_all_ports = map_lookup_elem(&SFC_SELECT_MAP, &select_key);
	if (select_entry_all_ports != NULL && (select_entry == NULL || select_entry_all_ports->prefix_len > select_entry->prefix_len))
		redir->path = select_entry_all_ports->path;
