...
#if !defined(ENABLE_GOOGLE_VPC) && !defined(GOOGLE_PERIMETER_FEATURES) && defined(MULTI_NIC_DEVICE_TYPE)
...
#endif /* ENABLE_PER_PACKET_LB && (!MULTI_NIC_DEVICE_TYPE || ENABLE_GOOGLE_SERVICE_STEERING || (TUNNEL_MODE && MULTI_NIC_DEVICE_TYPE == EP_DEV_TYPE_INDEX_MULTI_NIC_VETH)) */
}
...
skip_policy_enforcement:
	if (ret == CT_NEW) {
#if defined(ENABLE_NODEPORT) && (defined(ENABLE_IPSEC) || defined(ENABLE_GOOGLE_MULTI_NIC))
		...
		ct_state_new.node_port = ct_has_nodeport_egress_entry4(get_ct_map4(tuple),
								       tuple, NULL, false);
#endif /* ENABLE_NODEPORT && (ENABLE_IPSEC || ENABLE_GOOGLE_MULTI_NIC) */
		ct_state_new.src_sec_id = src_label;
		ct_state_new.from_tunnel = from_tunnel;
		ct_state_new.proxy_redirect = *proxy_port > 0;
